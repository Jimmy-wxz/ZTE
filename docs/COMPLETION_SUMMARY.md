...
| 文件 | 功能 | 状态 |
|------|------|------|
| `tests/test_check_chroma.py` | 验证 ChromaDB 结构 | ✅ 可用 |
| `tests/test_direct_search.py` | 直接测试 ChromaDB 搜索 | ✅ 可用 |
| `tests/test_check_metadata.py` | 检查数据库元数据 | ✅ 可用 |
| `tests/test_testdata_search.py` | 完整搜索测试（需 OpenAI Key） | ✅ 可用 |
| `scripts/test_startup.sh` | 一键启动测试环境 | ✅ 可用 |
| `tests/final_test.py` | 综合测试套件 | ✅ 可用 |
...
| `test_data/prompts/suggested_prompts.txt` | 可直接使用的提示词列表 | ✅ 完成 |
...
### Phase 1: 基础验证 (10 分钟)
1. 运行 `python tests/final_test.py` 验证系统状态
2. 启动后端服务器
3. 测试 3-5 个简单查询

### Phase 2: 功能测试 (30 分钟)
1. 使用 `test_data/prompts/suggested_prompts.txt` 中的查询进行测试
2. 记录每个查询的返回结果和相关性评分
