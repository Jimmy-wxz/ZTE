        ...
        for turn in range(0, self.max_turn):
            ...
            if parsed_resp is None:
                ...
                parsed_resp = {
                    ...
                    "think": "Parse failed, skipping this turn",
                    "action_think": "",
                    "search_querys": []
                }
            ...
            # Always perform web search (force fresh search to avoid empty cached results)
            actions_to_run = [(
                "BingBrowser.full_pipeline_search",
            ...
            # If a local knowledge base action is available, also retrieve from it
            if self._action_executor is not None and hasattr(self._action_executor, 'action_names'):
                if "LocalKnowledgeBase" in self._action_executor.action_names():
            ...
            merged_web_pages = []
            merged_result_parts = []
            for action, action_input in actions_to_run:
                ...
                if action_return.result:
                    if "web_pages" in action_return.result:
                        merged_web_pages.extend(action_return.result["web_pages"])
                    if action_return.result.get("result"):
                        merged_result_parts.append(str(action_return.result["result"]))

            # Re-index merged web_pages so global_index is contiguous
            for idx, page in enumerate(merged_web_pages, start=global_start_index):
                page["global_index"] = idx

            current_turn_info["tool_result"] = {
                "web_pages": merged_web_pages,
                "result": "\n\n".join(merged_result_parts) if merged_result_parts else "",
                "juege_and_summarized_search_results": merged_web_pages,
                "exclude_search_results": [],
            }

            # Update global start index
            page_cnt = len(merged_web_pages)
