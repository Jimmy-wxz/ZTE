...
import { generateReport, pingAPI, listKnowledgeBases } from '../utils/api';
import HistoryPanel from '../components/HistoryPanel';

// Recommended model options
const commonModels = [
  // NebulaCoder-V8.0 removed due to severe performance issues (15+ minutes per task)
  { label: 'DeepSeek-V4 (Fast & Recommended)', value: 'deepseek-chat' },
  { label: 'DeepSeek-V4-Pro (Best Quality)', value: 'deepseek-v4-pro' },
...
const ReportGenerationPage = () => {
  ...
  const [model, setModel] = useState('deepseek-chat'); // Default to DeepSeek for speed
  const [searchEngine, setSearchEngine] = useState('google');
  const [enableSearch, setEnableSearch] = useState(true); // Enabled by default with Google (requires SerpAPI key)
  ...
  // Check if API is available on component mount and load knowledge bases
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNebulacoderModel && !apiKeys.nebulacoder) {
      setError('Please provide your NebulaCoder API key in the settings section.');
      setShowApiSection(true);
      return;
    }
    
    // Google search is now the default - requires SerpAPI key
    // Users can switch to Bing if they don't have a SerpAPI key
    if (enableSearch && searchEngine === 'google' && !apiKeys.serpapi) {
      setError('Google Search is selected but no SerpAPI key was provided. Please add your SerpAPI key in the settings section, or switch to Bing search (which doesn\'t require an API key).');
      setShowApiSection(true);
      return;
    }

    if (useKnowledgeBase && !knowledgeBaseName) {
      setError('Please select a knowledge base or disable "Use Knowledge Base".');
      return;
    }
    ...
    try {
      ...
      const response = await generateReport({
        ...
        searchEngine,
        useKnowledgeBase,
        knowledgeBaseName: useKnowledgeBase ? knowledgeBaseName : null,
        apiKeys: {
        ...
        <form onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4}>
              ...
              <Box sx={{ mt: 1, mb: 1 }}>
                <Typography variant="caption" color="text.secondary" display="block">
                  💡 Tip: If your prompt contains a specific URL (e.g., https://example.com/article), the system will automatically fetch it directly.
                </Typography>
              </Box>

              <FormControl fullWidth disabled={!enableSearch}>
                ...
                >
                  <MenuItem value="google">Google (Default, Requires SerpAPI Key)</MenuItem>
                  <MenuItem value="bing">Bing (Free, No API Key Required)</MenuItem>
                </Select>
              </FormControl>
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              ...
              />
              <FormControl fullWidth disabled={!useKnowledgeBase} sx={{ mt: 1 }}>
                ...
                >
                  ...
                  {knowledgeBases.map((kb) => (
                    <MenuItem key={kb.name} value={kb.name}>
                      {kb.name} ({kb.status || 'pending'}, {kb.chunk_count || 0} chunks)
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                      <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                          <TextField
                            label="NebulaCoder API Key"
                            fullWidth
                            variant="outlined"
                            value={apiKeys.nebulacoder}
                            onChange={(e) => handleApiKeyChange('nebulacoder', e.target.value)}
                            type={showNebulacoderKey ? 'text' : 'password'}
                            placeholder="your-api-key..."
                            helperText="Required for NebulaCoder models"
                            InputProps={{
                              endAdornment: (
                                <InputAdornment position="end">
                                  <IconButton
                                    aria-label="toggle password visibility"
                                    onClick={() => setShowNebulacoderKey(!showNebulacoderKey)}
                                    edge="end"
                                  >
                                    {showNebulacoderKey ? <VisibilityOff /> : <Visibility />}
                                  </IconButton>
                                </InputAdornment>
                              ),
                            }}
