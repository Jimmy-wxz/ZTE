...
const LiveTaskList = ({ taskId, onTaskClick }) => {
  ...
  const fetchWorkspace = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/workspace/${taskId}`);

      if (!response.ok) {
        console.log('Workspace not available yet');
        setWorkspace('');
      ...
      setWorkspaceError('Content will be available once generation is complete');
      setWorkspace('');
      setWorkspaceLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchTaskGraph = async () => {
      try {
        ...
        if (!response.ok) {
          console.log('Task graph not available yet, waiting for WebSocket updates');
          setLoading(false);
          return;
        }
        const contentType = response.headers.get('content-type');
        if (!contentType || !contentType.includes('application/json')) {
          console.log('Received non-JSON response for task graph');
          setLoading(false);
          return;
        }
        const data = await response.json();
