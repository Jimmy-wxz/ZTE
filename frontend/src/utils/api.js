 ...
 * @param {boolean} params.useKnowledgeBase - Whether to use a knowledge base
 * @param {string} params.knowledgeBaseName - Name of the knowledge base to use
 * @returns {Promise} - Promise that resolves with generation result
 */
 ...
 * @param {string} taskId - ID of the generation task
 * @returns {Promise} - Promise that resolves with task graph data or null if not available
 */
export const getTaskGraph = async (taskId) => {
  try {
    const response = await apiClient.get(`/task-graph/${taskId}`);
    if (!response.data || !response.data.taskGraph) {
      console.log('Task graph data empty or missing');
      return null;
    }
    return response.data;
  } catch (error) {
    console.error('Error fetching task graph data:', error.message);
 ...
 * @param {string} taskId - ID of the generation task
 * @returns {Promise} - Promise that resolves with workspace content or empty string if not available
 */
export const getWorkspace = async (taskId) => {
  try {
    ...
    console.error('Error fetching workspace content:', error.message);
    // Return empty workspace instead of throwing for better UX
    return { workspace: '', error: 'Content not available yet' };
  }
};
 ...
 * @returns {Promise} - Promise that resolves with knowledge base list
 */
 ...
 * @returns {Promise} - Promise that resolves with knowledge base details
 */
export const getKnowledgeBase = async (name) => {
  try {
    const response = await apiClient.get(`/knowledge-base/${name}`);
    return response.data;
  } catch (error) {
...
export const deleteKnowledgeBase = async (name) => {
  try {
    const response = await apiClient.delete(`/knowledge-base/${name}`);
    return response.data;
  } catch (error) {
    console.error(`Error deleting knowledge base ${name}:`, error);
    throw error;
  }
};
 ...
 * @param {string} name - Name of the knowledge base
 * @returns {Promise} - Promise that resolves with indexing status
 */
export const indexKnowledgeBase = async (name) => {
  try {
    const response = await apiClient.post(`/knowledge-base/${name}/index`);
    return response.data;
  } catch (error) {
    console.error(`Error indexing knowledge base ${name}:`, error);
    throw error;
  }
};
 ...
 * @returns {Promise} - Promise that resolves with upload status
 */
export const uploadKnowledgeBaseFiles = async (name, files) => {
  try {
    const formData = new FormData();
    formData.append('knowledgeBaseName', name);
    ...
    console.error(`Error uploading files to knowledge base ${name}:`, error);
    throw error;
  }
};